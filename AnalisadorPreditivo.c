...
#include<string.h>
#define TSIZE 128
// a tabela [i] [j] armazena o índice de produção que deve ser aplicado na i-ésima variável se a entrada for j-ésima não-terminal
int table[100][TSIZE];
// armazena toda a lista de terminais do valor ASCII usado para indexar terminais terminais[i] = 1 significa que o caractere com valor ASCII é um terminal
char terminal[TSIZE];
// armazena todos os terminais
// apenas as letras maiúsculas de 'A' a 'Z' podem ser não terminais
// não terminais[i] significam que o iº alfabeto está presente, já que o não terminal é a gramática
char nonterminal[26];
//
struct product {
    char str[100];
...
// armazena o first de cada produção na forma A-> ß
char first_rhs[100][TSIZE];
// verifica se o símbolo é um não terminal
int isNT(char c) {
    return c >= 'A' && c <= 'Z';
}
...
// Calcula o FIRST(ß) para cada A->ß
void FIRST_RHS() {
    int i, j;
    int t = 0;
    while (t < no_pro) {
...
int main() {
    ...
    // mostrar o follow de cada variável
    printf("\n");
    for (i = 0; i < no_pro; ++i) {
        if (i == 0 || (pro[i - 1].str[0] != pro[i].str[0])) {
            char c = pro[i].str[0];
    ...
    // mostra o first de cada variável ß na forma de A->ß
    printf("\n");
    for (i = 0; i < no_pro; ++i) {
